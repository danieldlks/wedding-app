import { rowToTable } from "../_lib/db.js";
...
export async function onRequestGet({ request, env }) {
  ...
  if (assignmentRows.length === 0) return Response.json({ tables: [], mySeats: [] });
  ...
  return Response.json({ tables: tableRows.map(rowToTable), mySeats });
}
